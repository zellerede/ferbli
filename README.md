...
- **Hand score**: Each in-round hand gets a **`HandScore`**: **`figure`**, display **`score`**, and optional **`suiteSize`** (2–4 for `one-suite` only). **Ordering** between two seats uses **`compareHands`** from `@ferbli/rules` on each seat’s card list: pattern tier (high-card, then one-suite with two cards, ace-pair, one-suite with three cards, triplet, one-suite with four cards, quadruplet), then display `score` only — matching tier and score ties (`compareHands` returns `0`). Only an **empty** hand scores **0** on `score`.
- **Example**: Herz ace + Herz king + Herz nine + Schelle seven → **one-suite** with display score **30** (11 + 10 + 9 on hearts).
...
- **Round end**: After each completed or aborted hand, every **connected human who was in that hand** must send **`ack_round_result`** before the next deal (human **Deal** or bot auto-deal). The server exposes this on `RoomSnapshot` as `roundResultPending`, `roundResultRequiredSeats`, and `roundResultAckedSeats` (**`PROTOCOL_VERSION` 10**).
