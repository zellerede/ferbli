...
- **Hand score**: Each in-round hand gets a **`HandScore`** on the wire: **`figure`** (`quadruplet`, `triplet`, `one-suite`, `ace-pair`, `high-card`), a small **`score`** for display, and **`strength`** (higher wins). **Quadruplet** beats **triplet** beats **one-suite** with **three or four** suited cards beats **ace-pair** (always display score **22**) beats **one-suite** with **two** suited cards beats **high card** (display score = highest card’s points). Only an **empty** hand scores **0**.
...
- **Round end**: After each completed or aborted hand, every **connected human who was in that hand** must send **`ack_round_result`** before the next deal (human **Deal** or bot auto-deal). The server exposes this on `RoomSnapshot` as `roundResultPending`, `roundResultRequiredSeats`, and `roundResultAckedSeats` (**`PROTOCOL_VERSION` 8**).
